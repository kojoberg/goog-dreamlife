<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-10" x-data="{ 
            ...
            async checkDrugInteraction(product, index) {
                // Get currently selected product IDs (excluding the target index)
                const cartIds = this.medications
                    .filter((m, i) => i !== index && m.product_id)
                    .map(m => parseInt(m.product_id));
                
                if (cartIds.length === 0) {
                    return true; // No other drugs to check against
                }
                
                this.checkingInteraction = true;
                try {
                    ...
                    if (data.interactions && data.interactions.length > 0) {
                        this.interactionWarnings = data.interactions;
                        this.pendingDrugSelection = product;
                        this.pendingMedIndex = index;
                        this.showInteractionModal = true;
                        return false; // Block selection, wait for modal
                    }
                    return true; // Safe to proceed
                } catch (e) {
                    console.error('Interaction check failed', e);
                    return true; // Allow on error to not block workflow
                } finally {
                    this.checkingInteraction = false;
                }
            },
            applyDrugSelection(product, index) {
                this.medications[index].product_id = product.id;
                this.medications[index].name = product.name;
                this.medications[index].dosage = product.dosage || '';
                this.medications[index].route = product.drug_route || '';
                this.medications[index].price = parseFloat(product.unit_price || 0);
            },
            confirmInteractionOverride() {
                if (this.pendingDrugSelection && this.pendingMedIndex !== null) {
                    this.applyDrugSelection(this.pendingDrugSelection, this.pendingMedIndex);
                }
                                        ...
                                        <div class="col-span-1 md:col-span-2 relative">
                                            ...
                                            <div x-show="showResults && search.length > 0"
                                                ...
                                                <template
                                                    ...
                                                    <div @click="
                                                        (async () => {
                                                            const safe = await $data.checkDrugInteraction(product, index);
                                                            if (safe) {
                                                                $data.applyDrugSelection(product, index);
                                                                search = product.name;
                                                            }
                                                            showResults = false;
                                                        })()
        ...
        <div x-show="showInteractionModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto"
            role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div x-show="showInteractionModal" x-transition:enter="ease-out duration-300"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                    x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0"
                ...
                <div x-show="showInteractionModal" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                        <p class="text-gray-700 mb-4">Adding this medication may cause a dangerous interaction:</p>
                        <div class="space-y-3 max-h-64 overflow-y-auto">
                            <template x-for="warning in interactionWarnings" :key="warning.drug">
                                <div class="p-3 bg-red-50 border border-red-200 rounded-lg">
                                    <div class="font-bold text-red-800"
                                        x-text="warning.drug + ' (' + warning.severity + ')'"></div>
                                    <div class="text-sm text-red-700 mt-1" x-text="warning.description"></div>
                                </div>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3">
                        <button type="button" @click="confirmInteractionOverride()"
                        ...
                        <button type="button" @click="closeInteractionModal()"
        ...
        <div x-show="checkingInteraction" style="display: none;"
            ...
            <div class="bg-white p-4 rounded-lg shadow-lg flex items-center gap-3">
                ...
                <span class="font-medium text-gray-700">Checking for drug interactions...</span>
            </div>
        </div>
