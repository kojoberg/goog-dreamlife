<aside :class="sidebarCollapsed ? 'w-20' : 'w-64'"
    ...
    <nav class="flex-1 overflow-y-auto py-4 space-y-1 custom-scrollbar" :class="sidebarCollapsed ? 'px-2' : 'px-3'">
        ...
        @if(Auth::user()->role === 'lab_scientist')
            <a href="{{ route('patients.index') }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'"
                class="{{ request()->routeIs('patients*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }} flex items-center gap-3 py-2.5 rounded-lg transition-all group overflow-hidden">
                <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('patients*') ? 'text-white' : 'text-slate-400 group-hover:text-white' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
                <span x-show="!sidebarCollapsed"
                    class="font-medium whitespace-nowrap transition-opacity duration-200">Patients & Lab Results</span>
            </a>
        @endif
            ...
            <div x-data="{ open: {{ request()->is('admin*') || request()->is('crm*') || request()->is('backups*') || request()->is('audit*') || request()->is('users*') || request()->is('branches*') || request()->is('analytics*') ? 'true' : 'false' }} }"
                ...
                <div x-show="open && !sidebarCollapsed" x-transition class="mt-1 pl-10 space-y-1 overflow-hidden">
                    <a href="{{ route('admin.index') }}"
                        class="{{ request()->routeIs('admin.index') ? 'text-indigo-400' : 'text-slate-400 hover:text-white' }} block py-1.5 text-sm whitespace-nowrap">Admin
                        Console</a>
                    <a href="{{ route('admin.financials.index') }}"
                        class="{{ request()->routeIs('admin.financials*') ? 'text-indigo-400' : 'text-slate-400 hover:text-white' }} block py-1.5 text-sm whitespace-nowrap">Financial
                        Reports</a>
                    <a href="{{ route('admin.crm.index') }}"
                        class="{{ request()->routeIs('admin.crm*') ? 'text-indigo-400' : 'text-slate-400 hover:text-white' }} block py-1.5 text-sm whitespace-nowrap">CRM
                        & Messaging</a>
                    ...
                    <a href="{{ route('audit-logs.index') }}"
                        class="{{ request()->routeIs('audit-logs*') ? 'text-indigo-400' : 'text-slate-400 hover:text-white' }} block py-1.5 text-sm whitespace-nowrap">Audit
                        Logs</a>
